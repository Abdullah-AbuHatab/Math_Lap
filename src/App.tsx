...
import Shell, { AlgebraMode } from './components/layout/Shell';
...
export default function App() {
  ...
  const [interactionMode, setInteractionMode] = useState<'select' | 'pen' | 'eraser'>('select');
  ...
  return (
    <Shell
      ...
      onReset={() => { setResetTrigger(p => p + 1); setClearTrigger(p => p + 1); setZoom(1); }}
    ...
    >
      {activeTool === 'algebra-tiles'   && <AlgebraTiles  {...sharedProps} mode={algebraMode} />}
      {activeTool === 'base-ten-blocks' && <BaseTenBlocks {...sharedProps} />}
