...
export default function Workspace({
  ...
  useEffect(() => {
    const resize = () => {
      ...
      if (rect && c) { c.width = rect.width; c.height = rect.height; }
    };
    resize();
  ...
  const hScrollRatio = Math.max(0.05, containerW / (contentSize.w || containerW));
  const vScrollRatio = Math.max(0.05, containerH / (contentSize.h || containerH));
  const hScrollThumbW = Math.max(30, containerW * hScrollRatio - 4);
  const vScrollThumbH = Math.max(30, containerH * vScrollRatio - 4);
  const hScrollX = (clampedScrollX / maxScrollX) * (containerW - hScrollThumbW - 4);
  const vScrollY = (clampedScrollY / maxScrollY) * (containerH - vScrollThumbH - 4);
    ...
    if (isMarquee.current) {
      ...
      if (m && (m.w > 6 || m.h > 6)) {
        ...
        const hit = items.filter(item => {
          ...
          return !(item.position.x + iw < m.x || item.position.x > m.x + m.w ||
                   item.position.y + ih < m.y || item.position.y > m.y + m.h);
        });
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ transform: `translate(${pan.x - scrollPos.x}px, ${pan.y - scrollPos.y}px) scale(${zoom})` }}
      >
        {children}
        ...
        {marquee && (marquee.w > 4 || marquee.h > 4) && (
          <div
            style={{
              ...
              border: '2px solid #08b8fb', background: 'rgba(8,184,251,0.08)',
      ...
      {contentSize.w > containerW && (
        <div
          style={{
            position: 'absolute', bottom: 0, left: 0, right: 16,
            height: 12, background: '#f1f5f9', borderTop: '1px solid #e2e8f0',
          }}
          onClick={handleHorizontalScroll}
        >
          <div
            style={{
              position: 'absolute', height: 12, background: '#cbd5e1',
              borderRadius: 6, left: hScrollX, width: hScrollThumbW,
              cursor: 'grab', top: 0,
            }}
          />
        </div>
      ...
      {contentSize.h > containerH && (
        <div
          style={{
            ...
            width: 12, background: '#f1f5f9', borderLeft: '1px solid #e2e8f0',
          }}
          onClick={handleVerticalScroll}
        >
          <div
            style={{
              position: 'absolute', width: 12, background: '#cbd5e1',
              borderRadius: 6, top: vScrollY, height: vScrollThumbH,
              cursor: 'grab', left: 0,
            }}
          />
        </div>
