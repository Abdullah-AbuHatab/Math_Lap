...
function TileVisual({
  ...
  const visW = rotated ? base.h : base.w,
    visH = rotated ? base.w : base.h;
  ...
  return (
    <div
      style={{
        ...
        height: visH,
        position: "relative",
        outline: isSelected ? "2.5px solid #08b8fb" : "none",
        outlineOffset: 3,
        borderRadius: 4,
        display: "flex",
        alignItems: "center",
...
function TileMenu({
  ...
  const allLocked = selectedIds.every(
    (id) => items.find((i) => i.id === id)?.isLocked,
  );
  ...
  return (
    ...
    >
      <div style={{ paddingBottom: 4 }}>
        {canRotate &&
          lblBtn(onRotate, <RotateCw size={14} color="#64748b" />, "Rotate")}
        {lblBtn(
          onInvert,
          <span
            style={{
  ...
  return (
    ...
    >
      <div style={{ paddingBottom: 4 }}>
        ...
        {mat.kind === "factors" &&
          lblBtn(
            () => onUpdate({ factorShowLabels: !mat.factorShowLabels }),
            <Tag size={14} color="#64748b" />,
            "Labels On/Off",
          )}
                ...
                {SYMBOLS.map((s) => (
                  <button
                    ...
                    style={{
                      width: 34,
                      height: 34,
                      border: `2px solid ${s === mat.symbol ? "#08b8fb" : "#e2e8f0"}`,
                      borderRadius: 8,
                      cursor: "pointer",
                      background: s === mat.symbol ? "#eff9ff" : "#fff",
                      color: "#334155",
                      fontWeight: 700,
                      fontSize: 16,
                    }}
            ...
            {sub === "rows" && (
              ...
              >
                <button
                  ...
                  onClick={(e) => {
                    stop(e);
                    onUpdate({ rows: Math.max(1, mat.rows - 1) });
                  }}
                  style={{
                    width: 28,
                    height: 28,
                ...
                <button
                  ...
                  onClick={(e) => {
                    stop(e);
                    onUpdate({ rows: Math.min(6, mat.rows + 1) });
                  }}
                  style={{
                    width: 28,
                    height: 28,
...
function SolvingMatVisual({
  ...
  const toExpr = (sideItems: ManipulativeItem[]) => {
    ...
    sideItems.forEach((i) => {
      c[i.type] = (c[i.type] || 0) + (i.isNegative ? -1 : 1);
    });
    const p: string[] = [];
    if (c.x2)
      p.push(Math.abs(c.x2) === 1 ? (c.x2 < 0 ? "-x²" : "x²") : `${c.x2}x²`);
  ...
  const tilesInMat = tiles.filter(
    (t) =>
      t.position.x >= mat.position.x &&
      t.position.x <= mat.position.x + W &&
      t.position.y >= mat.position.y &&
      t.position.y <= mat.position.y + H,
  );
...
function FactorsMatVisual({
  ...
  const leftItems = tilesInMat.filter(
    (t) => relX(t) < TRACK && relY(t) >= TRACK,
  );
  const productItems = tilesInMat.filter(
    (t) => relX(t) >= TRACK && relY(t) >= TRACK,
  );
...
export default function AlgebraTiles({
  ...
  const effectiveSize = useCallback((item: ManipulativeItem) => {
    const { w, h } = TILE_SIZES[item.type as keyof typeof TILE_SIZES] ?? {
      w: 40,
      h: 40,
    };
  ...
  const snapOne = useCallback(
    (id: string, list: ManipulativeItem[]): ManipulativeItem[] => {
  ...
  const addMat = () => {
    ...
    const pos = lastMat
      ? {
          x: lastMat.position.x + (mode === "solving" ? 580 : 530),
          y: lastMat.position.y,
        }
      : { x: 40, y: 30 };
    setMats((prev) => [
      ...prev,
  ...
  const onTilePointerDown = (e: React.PointerEvent, id: string) => {
    ...
    items.forEach((i) => {
      if (ids.includes(i.id) && !i.isLocked) origins[i.id] = { ...i.position };
    });
  ...
  const checkZeroPair = (movedIds: string[]) => {
    setItems((cur) => {
      for (const mid of movedIds) {
        const m = cur.find((i) => i.id === mid);
        if (!m) continue;
        const opp = cur.find(
          (o) =>
            o.id !== mid &&
            o.type === m.type &&
            o.isNegative !== m.isNegative &&
            Math.abs(o.position.x - m.position.x) < 8 &&
            Math.abs(o.position.y - m.position.y) < 8,
        );
        if (opp) {
          setTimeout(() => animateZP(mid, opp.id), 0);
          break;
        }
      }
      return cur;
  ...
  const animateZP = async (id1: string, id2: string) => {
    setItems((prev) => {
      const a = prev.find((i) => i.id === id1),
        b = prev.find((i) => i.id === id2);
      if (!a || !b) return prev;
      const mx = (a.position.x + b.position.x) / 2,
        my = (a.position.y + b.position.y) / 2;
      return prev.map((i) =>
        i.id === id1 || i.id === id2 ? { ...i, position: { x: mx, y: my } } : i,
      );
    });
  ...
  const handleZPAction = async () => {
    ...
    const pos = sel.filter((i) => !i.isNegative),
      neg = sel.filter((i) => i.isNegative);
    ...
    setItems((prev) =>
      prev.map((item) => {
        const pair = pairs.find(
          (p) => p.a.id === item.id || p.b.id === item.id,
        );
        if (!pair) return item;
        return {
          ...item,
  ...
  const duplicateItems = (ids: string[]) => {
    const copies = items
      .filter((i) => ids.includes(i.id) && !i.isLocked)
      .map((i) => ({
        ...i,
        id: generateId(),
        position: { x: i.position.x + 8, y: i.position.y + 8 },
      }));
    setItems((prev) => [...prev, ...copies]);
    setSelectedIds(copies.map((c) => c.id));
  };
  const invertItems = (ids: string[]) =>
    setItems((prev) =>
      prev.map((i) =>
        ids.includes(i.id) && !i.isLocked
          ? { ...i, isNegative: !i.isNegative }
          : i,
      ),
    );
  const toggleLock = (ids: string[]) => {
    const al = ids.every((id) => items.find((i) => i.id === id)?.isLocked);
    setItems((prev) =>
      prev.map((i) => (ids.includes(i.id) ? { ...i, isLocked: !al } : i)),
    );
  };

  // ── mat ops ───────────────────────────────────────────────────────────────────
  const updateMat = (id: string, u: Partial<MatItem>) =>
  ...
  const resizeMat = (id: string, w: number, h: number) =>
    updateMat(id, { width: w, height: h });
    ...
    >
              ...
              return (
                <motion.div
                  ...
                  style={{
                    ...
                    cursor: interactionMode === "select" ? "grab" : "default",
                    touchAction: "none",
                  }}
      ...
      {selectedIds.length > 0 && menuItem && (
        <TileMenu
          ...
          onRotate={() =>
            updateItem(menuItem.id, {
              rotation: ((menuItem.rotation || 0) + 90) % 360,
            })
          }
          onInvert={() =>
            invertItems(selectedIds.length > 1 ? selectedIds : [menuItem.id])
          }
          onDuplicate={() =>
            duplicateItems(selectedIds.length > 1 ? selectedIds : [menuItem.id])
          }
          onLock={() =>
            toggleLock(selectedIds.length > 1 ? selectedIds : [menuItem.id])
          }
          onDelete={() =>
            deleteItems(selectedIds.length > 1 ? selectedIds : [menuItem.id])
          }
...
function Div() {
  return (
    <div
      style={{ height: 1, width: 40, background: "#e2e8f0", margin: "4px 0" }}
    />
  );
}
