...
interface ShellProps {
  ...
  // base-ten-blocks workspace mode
  baseTenMode?: BaseTenMode;
  onBaseTenModeChange?: (m: BaseTenMode) => void;
}
...
const BASE_TEN_MODES: { id: BaseTenMode; label: string }[] = [
  { id: "basic", label: "Basic" },
...
export default function Shell({
  ...
  const [baseTenModeMenuOpen, setBaseTenModeMenuOpen] = useState(false);
  ...
  const currentBaseTenMode = BASE_TEN_MODES.find((m) => m.id === baseTenMode);

  return (
    <div
      className="flex h-screen w-full bg-string-bg-alt font-sans"
    ...
    >
      ...
      >
        ...
        <div className="p-4 flex items-center gap-3 border-b border-string-border">
          ...
          {isSidebarOpen && (
            <span className="font-display font-bold text-xl tracking-tight text-string-navy">
              String
            </span>
          )}
        </div>
        ...
        <div className="p-4 border-t border-string-border space-y-1">
          ...
          {/* Workspace mode switcher — only shown for Base Ten Blocks */}
          {activeTool === "base-ten-blocks" && onBaseTenModeChange && (
            <div className="relative">
              <button
                onClick={() => setBaseTenModeMenuOpen((v) => !v)}
                className={cn(
                  "w-full flex items-center gap-3 p-3 rounded-xl transition-all",
                  baseTenModeMenuOpen
                    ? "bg-string-bg-alt text-string-blue"
              ...
              >
                ...
                {isSidebarOpen && (
                  <span className="font-medium">
                    {currentBaseTenMode?.label ?? "Basic"}
                  </span>
                )}
              </button>
              ...
              {baseTenModeMenuOpen && (
                <div
                  ...
                  style={{ minWidth: 140 }}
                >
                  {BASE_TEN_MODES.map((m) => (
                    <button
                      key={m.id}
                      onClick={() => {
                        onBaseTenModeChange(m.id);
                        setBaseTenModeMenuOpen(false);
                      }}
                      className="w-full flex items-center justify-between px-4 py-2.5 text-sm hover:bg-string-bg-alt transition-colors"
                      style={{
                        color: baseTenMode === m.id ? "#08b8fb" : "#334155",
                        fontWeight: baseTenMode === m.id ? 600 : 400,
                      }}
                    >
                      {m.label}
                      {baseTenMode === m.id && <Check size={14} />}
                    </button>
                  ))}
                </div>
              )}
      ...
      <main
        ...
        onClick={() => {
          ...
          if (baseTenModeMenuOpen) setBaseTenModeMenuOpen(false);
        }}
      >
        {/* Header */}
        <header className="h-16 bg-white border-b border-string-border flex items-center justify-between px-6 z-10 shrink-0">
          <h2 className="font-display font-bold text-lg text-string-navy">
            ...
            {activeTool === "base-ten-blocks" && currentBaseTenMode && (
              <span className="ml-2 text-sm font-normal text-string-navy-muted">
                — {currentBaseTenMode.label}
              </span>
            )}
          </h2>
        ...
        <div className="absolute bottom-6 right-6 flex flex-col gap-2 z-10">
          <div className="flex flex-col bg-white rounded-xl shadow-string-lg border border-string-border p-1">
            <button
              onClick={onZoomIn}
              className="p-2 rounded-lg hover:bg-string-bg-alt text-string-navy-muted"
              title="Zoom In"
            >
              <Plus size={20} />
            </button>
